...
export default function StudentCard({ student, onClick }) {
  const getStatusConfig = () => {
    ...
    switch (student.statusColor) {
      ...
      default:
        return { border: 'border-slate-800', text: 'text-slate-400', bg: 'bg-slate-900/40', statusClass: '' }
    }
  }
  ...
  return (
    <div
      ...
      className={`group relative prof-panel p-4 overflow-hidden transition-all duration-200 ${student.activityStatus === 'TERMINATED' ? 'opacity-75 cursor-not-allowed' : 'cursor-pointer hover:border-slate-700'
        }`}
    >
      {/* Header */}
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="space-y-1">
          ...
          <p className="text-xs font-medium text-slate-300">{student.department || 'N/A'}</p>
        </div>
        <div className="space-y-1 text-right">
          <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Last Activity</p>
          <p className="text-xs font-medium text-slate-300">{formatLastActivity(student.lastActivity)}</p>
        </div>
      </div>
