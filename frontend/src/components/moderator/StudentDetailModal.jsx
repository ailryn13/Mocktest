          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-500 mt-1">
                Candidate ID: <span className="font-mono text-slate-400">{student.studentId}</span>
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden aspect-video relative group">
                ...
                <div className="h-full w-full flex flex-col items-center justify-center bg-slate-950/50">
                  <Camera size={40} className="text-slate-800 mb-2" />
                  <p className="text-[10px] font-medium text-slate-600 uppercase tracking-widest">Feed Initializing...</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 space-y-3">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-500">Last Activity Detected</span>
                    <span className="text-slate-300 font-medium tabular-nums">{formatTime(student.lastActivity)}</span>
                  </div>
                  <div className="h-px bg-slate-800" />
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-500">Session Join Time</span>
                    <span className="text-slate-300 font-medium tabular-nums">{formatTime(student.lastHeartbeat)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              {student.activityStatus !== 'TERMINATED' ? (
                <div className="space-y-6">
                  {/* Warning */}
                  <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 border-l-4 border-l-amber-500/50">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-slate-900 border border-slate-800 rounded-lg text-sm text-white placeholder:text-slate-600 focus:border-amber-500/50 transition-all resize-none"
                      rows={3}
                    />
                    <button
                  ...
                  {/* Termination */}
                  <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-5 border-l-4 border-l-rose-500/50">
                    ...
                    {!showTerminateConfirm ? (
                      ...
                      <div className="space-y-3">
                        <textarea
                          ...
                          className="w-full px-4 py-3 bg-slate-900 border border-rose-500/30 rounded-lg text-sm text-white placeholder:text-rose-900/50 focus:border-rose-500 transition-all resize-none"
                          rows={2}
                        />
