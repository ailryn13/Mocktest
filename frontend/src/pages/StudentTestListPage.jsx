            ...
            <div className="max-w-7xl mx-auto p-6 relative z-10">
                ...
                <div className="prof-panel p-6 mb-8 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Welcome, {user?.name || 'Student'}</p>
                    </div>
                    ...
                    {activeTab === 'ongoing' ? (
                        ...
                        <div className="space-y-6">
                            ...
                            {ongoing.length > 0 ? (
                                ...
                                <div className="prof-card p-12 text-center bg-slate-900/40 border-dashed border-2 border-white/5">
                                    <div className="text-3xl mb-4 opacity-20">🎯</div>
                                    <p className="text-slate-500 font-medium">No ongoing assessments at the moment</p>
                                </div>
                            )}
                        </div>
                    ) : (
                        ...
                        <div className="space-y-6">
                            ...
                            {completedTests.length > 0 ? (
                                ...
                                <div className="prof-card p-12 text-center bg-slate-900/40 border-dashed border-2 border-white/5">
                                    <div className="text-3xl mb-4 opacity-20">🏆</div>
                                    <p className="text-slate-500 font-medium">No completed assessments yet</p>
                                </div>
                            )}
                        </div>
                    )}
                ...
                <div className="mt-16 pt-10 border-t border-white/5">
                    ...
                    {upcoming.length > 0 && (
                        <section className="mb-10">
                            <h2 className="text-lg font-semibold text-slate-400 mb-6 flex items-center gap-2">
                    ...
                    {expired.length > 0 && (
                        <section className="opacity-75">
                            <h2 className="text-lg font-semibold text-slate-500 mb-6 flex items-center gap-2">
                    ...
                    {tests.length === 0 && (
                        <div className="prof-panel p-16 text-center">
                            ...
                            <p className="text-slate-400 max-w-sm mx-auto">There are currently no scheduled tests available for your department. Please check back later.</p>
                        </div>
                    )}
                </div>
            </div>
        ...
        <div className="prof-card p-6 flex flex-col h-full">
            <div className="flex justify-between items-start mb-6">
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded">
                            {test.type?.replace('_ONLY', '')}
                        </span>
                        <span className="px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-slate-800 text-slate-400 border border-white/5 rounded">
            ...
            <div className="space-y-3 mb-6 mt-auto">
                <div className="flex justify-between items-center text-xs py-2 border-b border-white/5">
                    ...
                    <span className="text-slate-300 font-medium">{formatDate(test.startDateTime)}</span>
                </div>
                <div className="flex justify-between items-center text-xs py-2 border-b border-white/5">
                    <span className="text-slate-500">Deadline</span>
                    <span className="text-slate-300 font-medium">{formatDate(test.endDateTime)}</span>
                </div>
            </div>

            {status === 'active' && (
                <div className="space-y-4">
                    <div className="text-center bg-slate-900/50 py-2 rounded border border-white/5">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Time Remaining</span>
            ...
            {status === 'upcoming' && (
                <div className="p-3 bg-slate-900/50 rounded border border-white/5 text-center text-xs text-slate-400">
            ...
            {status === 'expired' && (
                <button
                    className="w-full px-4 py-2 rounded-lg bg-slate-800/50 text-slate-600 font-medium cursor-not-allowed border border-white/5"
                    disabled
                >
                    Session Unavailable
                </button>
